    ...
    <van-popup v-model="detailsShow" position="right" class="my-popup" safe-area-inset-bottom get-container="#app">
      <div style="height: 100%">
        ...
        <div class="my-main-content">
          ...
          <van-row>
            <content-body title="专利需求内容"
                          :contentArr="[
                          {content:item.devBackground,littleTitle:'研发背景',isHtml:false},
                          {content:item.patentTarget,littleTitle:'攻关目标',isHtml:false},
                          {content:item.resultShow,littleTitle:'成果形式',isHtml:true}
                          ]"
                          v-if="item.devBackground || item.patentTarget || item.resultShow"></content-body>
          </van-row>

          <van-row>
            <content-body title="专利需求描述"
                          :contentArr="[
                          {content:item.technicalProblemContent,littleTitle:'技术难题主要内容',isHtml:false},
                          {content:item.qqyjkfjc,littleTitle:'前期研究开发基础',isHtml:false},
                          {content:item.schyfsb,littleTitle:'生产和研发设备',isHtml:true}
                          ]"
                          v-if="item.technicalProblemContent || item.qqyjkfjc || item.schyfsb"></content-body>
          </van-row>
        </div>
